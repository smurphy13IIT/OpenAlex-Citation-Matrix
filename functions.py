...
def cited_by_per_decade(all_articles, pub_year, type_limiter, journal_counts, journal_titles):
    ...
    for j in all_work_ids:
        search_url_stem = "https://api.openalex.org/works?filter=cites:"
        all_cited_by.extend(fetch_all_articles_cursor(search_url_stem + j + ",publication_year:" + pub_year + type_limiter + "&per_page=100&mailto=smurphy13@iit.edu"))
...
def references_per_decade(all_articles, pub_year, type_limiter, journal_counts, journal_titles):
    ...
    for k in references:
        ref_response = requests.get("https://api.openalex.org/works?filter=ids.openalex:" + k + ",publication_year:" + pub_year + type_limiter +"&per_page=100&mailto=smurphy13@iit.edu")
